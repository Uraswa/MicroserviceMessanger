...
class ApplicationCache {
    ...
    async _getChatMemberViaHttp(chat_id, user_id) {
        let response = await InnerCommunicationService.get('/api/chats/getChatMember?chat_id=' + chat_id + "&user_id=" + user_id);
    ...
    async _getChatMembersViaHttp(chat_id) {
        let response = await InnerCommunicationService.get('/api/chats/getChatMembers?chat_id=' + chat_id);
        if (response.status === 200 && response.data.success) {
